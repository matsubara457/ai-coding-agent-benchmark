...
async fn update_todo(
    ...
    // Check if the todo exists and get current values
    ...
    match existing {
        Ok(current) => {
            let new_title = body.title.clone().unwrap_or(current.title);
            let new_completed = body.completed.unwrap_or(current.completed);
